...
func (j *JWK) MarshalJSON() ([]byte, error) {
	return json.Marshal(j.marshal)
}
func (j *JWK) UnmarshalJSON(raw []byte) error {
	return json.Unmarshal(raw, &j.marshal)
}
func (j *JWK) Validate() error {
	...
	if !j.marshal.KTY.valid() {
		return fmt.Errorf(`%w: marhsal is required to have a valid Key Type ("kty" parameter): %q is invalid or unrecognized`, ErrJWKValidation, j.marshal.KTY)
	}
