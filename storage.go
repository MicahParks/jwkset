...
func NewMemoryStorage() Storage {
	return &memoryJWKSet{
		m: make(map[string]KeyWithMeta),
	}
}

func (m *memoryJWKSet) SnapshotKeys(ctx context.Context) ([]KeyWithMeta, error) {
	m.mux.RLock()
	defer m.mux.RUnlock()
	cpy := make([]KeyWithMeta, len(m.m))
	i := 0
...
func (m *memoryJWKSet) DeleteKey(ctx context.Context, keyID string) (ok bool, err error) {
	m.mux.Lock()
	defer m.mux.Unlock()
	_, ok = m.m[keyID]
	delete(m.m, keyID)
...
func (m *memoryJWKSet) ReadKey(ctx context.Context, keyID string) (KeyWithMeta, error) {
	m.mux.RLock()
	defer m.mux.RUnlock()
	meta, ok := m.m[keyID]
	if !ok {
		return KeyWithMeta{}, fmt.Errorf("%s: %w", keyID, ErrKeyNotFound)
	}
	return meta, nil
}

func (m *memoryJWKSet) WriteKey(ctx context.Context, meta KeyWithMeta) error {
	m.mux.Lock()
	defer m.mux.Unlock()
	m.m[meta.KeyID] = meta
	return nil
